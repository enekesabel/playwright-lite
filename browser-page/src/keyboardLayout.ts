...
export type KeyboardKeyDescription = {
  code: string;
  key: string;
  keyCode: number;
  keyCodeWithoutLocation: number;
  location: number;
  text: string;
  shifted?: KeyboardKeyDescription;
};
...
/** Mirrors pinned `server/input.ts` buildLayoutClosure. */
export const keyboardLayout = (() => {
  const result = new Map<string, KeyboardKeyDescription>();
  for (const code in USKeyboardLayout) {
    const definition = USKeyboardLayout[code]!;
    const description: KeyboardKeyDescription = {
      code,
      key: definition.key || "",
      keyCode: definition.keyCode || 0,
      keyCodeWithoutLocation:
        definition.keyCodeWithoutLocation || definition.keyCode || 0,
      location: definition.location || 0,
      text: definition.text || "",
    };
    if (definition.key.length === 1) description.text = description.key;
    let shifted: KeyboardKeyDescription | undefined;
    if (definition.shiftKey) {
      shifted = {
        ...description,
        key: definition.shiftKey,
        text: definition.shiftKey,
      };
      if (definition.shiftKeyCode) shifted.keyCode = definition.shiftKeyCode;
    }
    result.set(code, { ...description, shifted });
    ...
    if (shifted) result.set(shifted.key, { ...shifted, shifted: undefined });
  }
  return result;
})();
