...
describe("pointer action compatibility", () => {
  it("resolves Page selectors non-strictly but keeps Locators strict", async () => {
    ...
    await expect(page.locator("button").click()).rejects.toThrow("strict mode violation");
    await expect(page.click("button", { strict: true })).rejects.toThrow("strict mode violation");
  ...
  it("keeps ElementHandles fixed while Locators retry replacement nodes", async () => {
    ...
    expect((await fixed as Error).message).toContain("Element is not attached to the DOM");
    await resolved;
  ...
  it("dispatches right and middle buttons without a primary click", async () => {
    ...
    for (const name of ["mousedown", "mouseup", "contextmenu", "click", "auxclick"])
    ...
    expect(events).toEqual([
      ["mousedown", 2, 2], ["contextmenu", 2, 2], ["mouseup", 2, 0], ["auxclick", 2, 0],
      ["mousedown", 1, 4], ["mouseup", 1, 0], ["auxclick", 1, 0],
    ]);
  });
  ...
  it("restores held modifiers after overrides and trial runs", async () => {
    ...
    const button = document.querySelector("button")!;
    button.addEventListener("click", (event) => modifiers.push([event.ctrlKey, event.shiftKey]));
    button.addEventListener("keydown", (event) => keyboardEvents.push(event.key));
    ...
    expect(modifiers).toEqual([[false, true], [true, false], [true, false]]);
  ...
  it("preserves click counts, delay, pressure and microtask ordering", async () => {
    ...
    button.addEventListener("pointerdown", (event) => pressures.push(event.pressure));
    button.addEventListener("pointerup", (event) => pressures.push(event.pressure));
    button.addEventListener("mousedown", () => {
      downAt = performance.now();
      microtaskRan = false;
      queueMicrotask(() => queueMicrotask(() => { microtaskRan = true; }));
    });
  ...
  it("force bypasses hit checks, not geometry or actual event targeting", async () => {
    document.body.innerHTML = '<button style="position:fixed;left:30px;top:30px;width:80px;height:40px">go</button><div id="cover" style="position:fixed;inset:0;z-index:10"></div>';
    const page = createPage();
    let buttonClicks = 0, coverClicks = 0;
    document.querySelector("button")!.addEventListener("click", () => buttonClicks++);
    document.querySelector("#cover")!.addEventListener("click", () => coverClicks++);
    await page.locator("button").click({ force: true });
    expect([buttonClicks, coverClicks]).toEqual([0, 1]);
    document.querySelector("button")!.style.display = "none";
    await expect(page.click("button", { force: true })).rejects.toThrow("Element is not visible");
    expect([buttonClicks, coverClicks]).toEqual([0, 1]);
  });

  it("honors scroll:none for clicks and hover with positions and modifiers", async () => {
    document.body.innerHTML = '<div style="height:120px;overflow:auto"><button style="margin-top:800px">go</button></div>';
    const page = createPage();
    const container = document.querySelector("div")!;
    await expect(page.locator("button").click({ scroll: "none", force: true })).rejects.toThrow("outside of the viewport");
    expect(container.scrollTop).toBe(0);
    await page.locator("button").hover({ modifiers: ["Shift"], position: { x: 4, y: 4 } });
  ...
  it("does not dispatch delayed activation after timeout and restores modifiers", async () => {
    ...
    await expect(page.click("button", { delay: 500, timeout: 150, modifiers: ["Shift"] })).rejects.toThrow("Timeout 150ms exceeded");
    await new Promise((resolve) => setTimeout(resolve, 550));
    expect(events).toEqual(["mousedown"]);
    let shifted = true;
    button.addEventListener("click", (event) => { shifted = event.shiftKey; });
    await page.click("button");
    expect(shifted).toBe(false);
  });

  it("intercepts layout changes caused by hover before activating", async () => {
    document.body.innerHTML = '<button style="position:fixed;left:30px;top:30px;width:80px;height:40px">go</button>';
    const page = createPage();
    let clicks = 0;
    const button = document.querySelector("button")!;
    ...
    await expect(page.click("button", { timeout: 200 })).rejects.toThrow("Timeout 200ms exceeded");
