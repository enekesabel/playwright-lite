/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-function-type, no-useless-assignment -- kept aligned with Playwright's pinned Jest-derived matcher source */
/**
...
function getType(value: unknown): string {
  ...
  if (typeof value === "object") {
    if ((value as object).constructor === RegExp) return "regexp";
    if ((value as object).constructor === Map) return "map";
    if ((value as object).constructor === Set) return "set";
    if ((value as object).constructor === Date) return "date";
    return "object";
  }
...
export function buildCustomAsymmetricMatcher(
  matcherName: string,
  ...
  const positive = (...sample: [unknown, ...Array<unknown>]) =>
    new CustomMatcher(false, ...sample);
  const inverse = (...sample: [unknown, ...Array<unknown>]) =>
...
const printExpectedConstructorNameNot = (
  label: string,
  expected: Function
...
export const createThrowMatcher = (
  matcherName: string,
  fromPromise?: boolean
): RawMatcherFn =>
  function (
    ...
    if (expected === undefined) return toThrow(matcherName, options, thrown);
    if (typeof expected === "function")
...
type AsymmetricMatcherExpected = {
...
export function createExpectedPromiseMessage(
  matcherName: string,
  isNot: boolean,
...
export function createExpectedToResolveMessage(
  matcherName: string,
  isNot: boolean,
  promise: "resolves" | "rejects",
  actual: unknown
...
export function createExpectedToRejectMessage(
  matcherName: string,
  isNot: boolean,
  promise: "resolves" | "rejects",
  actual: unknown
