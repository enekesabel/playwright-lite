...
5. The promotion command reruns each candidate alone with that method sabotaged: its in-browser adapter dispatch throws instead of executing. A test that still passes proves nothing about the method and is refused. A withheld public matcher fails with its marker as the only text a test can read, in the thrown message and in `matcherResult` alike, and the matcher rerun accepts only a failure that shows that marker. Errors the rerun reports outside any test do not decide the verdict: only a test that failed on its own is judged. The rerun passes the method as a fixture option in a configuration it generates for that run alone; no environment variable is involved, so the switch is off in every ordinary run and no spec can set it.
