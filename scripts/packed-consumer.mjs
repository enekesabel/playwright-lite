...
let browser;
try {
  assert.ok(process.argv.length <= 3, "Usage: node scripts/packed-consumer.mjs [package.tgz]");
  ...
  execFileSync("npm", ["install", "--engine-strict", "--ignore-scripts", "--no-audit", "--no-fund", "--registry=https://registry.npmjs.org"], { cwd: temporary, stdio: "inherit", env });
  const consumerRequire = createRequire(resolve(temporary, "package.json"));
  const installedRoot = resolve(temporary, "node_modules/@enekesabel/playwright-lite");
  const [packed] = JSON.parse(execFileSync("npm", ["pack", "--dry-run", "--ignore-scripts", "--json"], { cwd: installedRoot, encoding: "utf8", env }));
  const files = new Set(packed.files.map(file => file.path));
  for (const file of ["dist/index.mjs", "dist/index.d.mts", "LICENSE", "README.md", "THIRD_PARTY_NOTICES.txt", "LICENSES/PLAYWRIGHT-LICENSE.txt", "LICENSES/PLAYWRIGHT-NOTICE", "LICENSES/YAML-LICENSE.txt"]) {
  ...
  assert.throws(() => consumerRequire.resolve("yaml"), { code: "MODULE_NOT_FOUND" });
  assert.throws(() => consumerRequire.resolve("@enekesabel/playwright-lite/dom"), { code: "ERR_PACKAGE_PATH_NOT_EXPORTED" });
  const declarations = readFileSync(resolve(installedRoot, "dist/index.d.mts"), "utf8");
  assert.doesNotMatch(declarations, /\b(?:AdapterJSHandle|LOCATOR_BRAND|isPlaywrightLiteLocator|resolveLocatorElements)\b/);
  ...
  execFileSync(process.execPath, [consumerRequire.resolve("typescript/bin/tsc"), "--project", resolve(temporary, "tsconfig.json")], { cwd: temporary, stdio: "inherit" });
  ...
  console.log(`PASS packed consumer on Node ${process.version}: strict isolated install, minimal exports, declarations, POM actions, keyboard, test IDs, and snapshots without an installed YAML dependency`);
} finally {
  await browser?.close();
