...
// Each promotion run gets its own directory: a Playwright run clears the
// output directory it writes to, and the corpus report and diagnostics of this
// promotion must survive the sabotage rerun. A run's generated config sits
// beside its output directory rather than inside it, for the same reason.
const PROMOTION_CORPUS_DIR = resolve(PKG_ROOT, "test-results/promotion");
 ...
 *
 * @param {string} [configPath]  A generated promotion configuration to run
 *   with instead of the package configuration.
 */
function runCorpus(configPath) {
...
/**
 * Write the configuration one promotion run uses: the package configuration,
 * with the given fixture options as literals in its `use` block, and its
 * output in `dir`. Workers re-evaluate this file, so the options never travel
 * through the environment, where a spec could set them.
 *
 * Every promotion run sets `knownFailureTimeout` to `null`: a candidate is
 * still outside the reviewed baseline, and the fixture would otherwise cap it
 * at the short timeout ordinary runs give expected failures, so a candidate
 * that needs longer could never pass, nor fail for the reason its sabotaged
 * rerun needs.
 *
 * @param {string} dir  The run's directory; the config is written there.
 * @param {Record<string, unknown>} options  Fixture options for this run.
 * @returns {string} The configuration's path.
 */
function writePromotionConfig(dir, options) {
  const configPath = resolve(dir, "playwright.config.ts");
  const use = { knownFailureTimeout: null, ...options };
  mkdirSync(dir, { recursive: true });
  writeFileSync(
    configPath,
    [
      "// Generated by scripts/upstream-baseline.mjs for one promotion run.",
      ...
      `  outputDir: ${JSON.stringify(resolve(dir, "output"))},`,
      `  use: { ...config.use, ${Object.entries(use)
...
function runSabotaged(entry, method, matcher) {
  ...
  const configPath = writePromotionConfig(SABOTAGE_DIR, {
    sabotagedMethod: matcher ? null : method,
    sabotagedMatcher: matcher ?? null,
  });
  runPlaywright(
    [
      `--config=${configPath}`,
...
if (isMain) {
  ...
  switch (command) {
    case "promote": {
      ...
      const entries = runCorpus(writePromotionConfig(PROMOTION_CORPUS_DIR, {}));
      doUpdate(entries);
      break;
    }
    case "check": {
